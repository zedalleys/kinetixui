...
**Presets reach web CSS and SwiftUI colours.**
[`kinetixui preset swiftui`](/docs/cli) resolves a preset into a `KinetixColors`
pair for [the SwiftUI package](/docs/swiftui) — colours only, because that is the
one axis it is themeable along; a design's radius and surface treatment apply on
the web and have no token to land on natively. Compose and Flutter have no
exporter yet. The preset format is platform-neutral so that they can, without a
new code format.
