...
A theme generated in [Create](/create) derives the same two states the same way, with one constraint
...
browser — and [`kinetixui preset swiftui`](/docs/cli) into a SwiftUI theme file. **Copy CSS** remains a different artifact from the preset itself: a
...
**Presets reach web CSS, SwiftUI colours and Compose colours.**
[`kinetixui preset swiftui`](/docs/cli) and
[`kinetixui preset compose`](/docs/cli) resolve a preset into a `KinetixColors`
pair for [SwiftUI](/docs/swiftui) and [Compose](/docs/compose) — colours only in
both, because that is the one axis either package is themeable along; a design's
radius and surface treatment apply on the web and have no token to land on
natively. Flutter has no exporter yet. The preset format is platform-neutral so
that it can, without a new code format.
