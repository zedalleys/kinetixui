...
Both commands use the same codec the website does, so a code means one thing in
both places. Neither executes anything from a preset — a preset is enum names,
hex colours and token names off a fixed list, and anything else is refused.

**There is no `preset apply`.** Turning a preset into CSS needs the theme engine,
which lives in the website today; `theme build` remains the way to compile a file
of literal colours from the command line. To get CSS from a preset, open it in
Create and use Copy CSS.
