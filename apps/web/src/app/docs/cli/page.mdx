...
design's radius and surface treatment do not travel. Two colours do not either:
`input` and `ring` are real tokens on the web and in SwiftUI, and `KinetixColors`
has no field for them. The generated file names all of this in its own header.
