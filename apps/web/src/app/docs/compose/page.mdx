...
The [SwiftUI](/docs/swiftui) port is at its kickoff stage (theme layer +
`KinetixButton` + CI). Flutter isn't started — the token engine emits
Dart, but there's no `Kinetix*` library for it yet.
