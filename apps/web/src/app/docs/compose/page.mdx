...
full React surface too (SwiftUI carries 69 `Kinetix*` views, Flutter 68;
alongside `Form`, `NavigationMenu` and `Combobox`, Compose skips `Chart`,
SwiftUI/Flutter skip `Inform`, and Flutter skips `Sidebar`). All four
platforms are at parity — see [Contributing](/docs/contributing) for the
rule that keeps
them there.
