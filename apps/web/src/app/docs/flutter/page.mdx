...
Write that pair by hand, or export one from a [Create](/create) design:

```bash
kinetixui preset flutter KX1_… --name AcmeTheme --output lib/acme_theme.dart
```

That writes a Dart class with a `light` and a `dark` `KinetixColors`, built from
the same resolved theme the workspace's Copy CSS renders — see
